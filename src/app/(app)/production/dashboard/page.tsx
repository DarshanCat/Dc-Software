...
export default async function ProductionDashboardPage() {
  const [draftDcs, pendingApproval, approvedDcs] = await Promise.all([
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-slate-200 pb-4 gap-3">
        <div>
          <h1 className="text-xl font-bold text-slate-900">Production DC Operations Portal</h1>
          <p className="text-xs text-slate-500">
            Create new delivery challans, submit for approval, and track authorized job work orders.
          </p>
        </div>
        <Link
          href="/dcs/new"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="rounded-lg border border-slate-200 bg-slate-50 p-4">
          <p className="text-xs font-semibold uppercase tracking-wider text-slate-700">1. My Draft DCs</p>
        ...
        <div className="rounded-lg border border-blue-200 bg-blue-50/50 p-4">
          <p className="text-xs font-semibold uppercase tracking-wider text-blue-800">3. Approved / Ready for Gate</p>
          <p className="mt-2 text-3xl font-extrabold text-blue-950">{approvedDcs.length}</p>
          <p className="mt-1 text-[11px] text-blue-700">Status: APPROVED</p>
        </div>
      </div>

      {/* Queue 1: Pending Approvals */}
      <div className="rounded-lg border border-slate-200 bg-white overflow-hidden shadow-sm">
        <div className="bg-amber-900 px-4 py-3 text-white flex items-center justify-between">
          <h2 className="text-sm font-bold uppercase tracking-wider">Pending Approval Delivery Challans</h2>
          <span className="rounded bg-amber-800 px-2 py-0.5 text-xs font-semibold">{pendingApproval.length} Pending</span>
        </div>
        <table className="w-full text-left text-xs">
          ...
          <tbody className="divide-y divide-slate-200">
            {pendingApproval.length === 0 ? (
              <tr>
                <td colSpan={6} className="p-6 text-center text-slate-400 italic">No DCs currently pending approval.</td>
              </tr>
            ) : (
              pendingApproval.map((dc) => (
                <tr key={dc.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3 text-center">
                    <Link
                      href={`/dcs/${dc.id}`}
                      className="inline-block rounded bg-amber-700 px-3 py-1 text-xs font-bold text-white hover:bg-amber-800"
                    >
                      View / Approve
                    </Link>
                  </td>
