...
/**
 * Dispatch (spec §20): APPROVED -> DISPATCHED. Creates an immutable material-out
...
export async function dispatchDc(dcId: string, input: DispatchInput = {}): Promise<ActionResult> {
  ...
  // Spec §20 dispatch guards: must be approved, must have at least one line,
  // quantities must be valid, and it must not already have a dispatch record
  // (dispatch is a one-time, immutable transaction — never re-run or overwritten).
